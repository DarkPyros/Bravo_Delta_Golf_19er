...
unsigned char packedData[PACKED_BYTES] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 
						 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0xA5, 0xBD, 0xC3, 0xDB, 0xE7};
...
//else modeFlag = PLAYBACK;
	...
	if(modeFlag != currentMode) {
		...
		if(modeFlag == PLAYBACK) {
			...
			SCH_addTask(TASKS_readFromSPI, 69, 	FRAME_PERIOD, CO_OP);
			SCH_addTask(TASKS_writeToSPI,  50, 	FRAME_PERIOD, CO_OP);
		}	
